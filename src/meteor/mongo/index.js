...
Object.assign(Mongo.Collection.prototype, {
  ...
  _ensureIndex(index, options) {
    ...
    if (self._collection.createIndex) {
      ...
      console.debug(`_ensureIndex has been deprecated, please use the new 'createIndex' instead${options?.name ? `, index name: ${options.name}` : `, index: ${JSON.stringify(index)}`}`)
  ...
  createIndex(index, options) {
    ...
    try {
      ...
      if (e.message.includes('An equivalent index already exists with the same name but different options.') && Meteor.settings?.packages?.mongo?.reCreateIndexOnOptionMismatch) {
        ...
        console.info(`Re-creating index ${index} for ${self._name} due to options mismatch.`);
