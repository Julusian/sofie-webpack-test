/**
 * Stolen from https://github.com/nrkno/sofie-core/blob/master/packages/server-core-integration/src/lib/ddpClient.ts
 * 
 * DDP client. Based on:
 *
...
export interface TLSOpts {
	...
	ca?: string[] // example: [ fs.readFileSync('server-cert.pem') ]

	/* Necessary only if the server requires client certificate authentication.*/
	key?: string // example: fs.readFileSync('client-key.pem'),
	cert?: string // example: fs.readFileSync('client-cert.pem'),
...
export class DDPClient extends EventEmitter<DDPClientEvents> {
	...
	private makeWebSocketConnection(url: string): void {
		...
		this.socket = new WebSocket(url) // , null, { tls: this.tlsOpts })
